...
BEGIN
    ...
    BEGIN TRY
        ...
        INSERT INTO silver.crm_cust_info
            (cst_id, cst_key,cst_firsname, cst_lastname, cst_matrital_status, cst_gndr, cst_create_date)
        SELECT
            cst_id,
            cst_key,
            TRIM(cst_firsname) cst_firsname,
            TRIM(cst_lastname) cst_lastname,

            CASE WHEN UPPER(TRIM(cst_matrital_status)) = 'M' THEN 'Married'
                WHEN UPPER(TRIM(cst_matrital_status)) = 'S' THEN 'Single'
                ELSE 'n/a'
            END cst_matrital_status,
        ...
        SELECT
            ...
            END sls_order_dt, -- Handling invalid dates and casting to dates

            CASE WHEN sls_ship_dt <= 0 OR LEN(sls_ship_dt) != 8 THEN NULL 
                ELSE CAST(CAST(sls_ship_dt AS VARCHAR) AS DATE)
            END sls_ship_dt, -- Handling invalid dates and casting to dates

            CASE WHEN sls_due_dt <= 0 OR LEN(sls_due_dt) != 8 THEN NULL 
                ELSE CAST(CAST(sls_due_dt AS VARCHAR) AS DATE)
            END sls_due_dt, -- Handling invalid dates and casting to dates 

            CASE WHEN sls_sales IS NULL OR sls_sales <= 0 OR sls_sales != sls_quantity * ABS(sls_price)
                THEN sls_quantity * ABS(sls_price)
                ELSE sls_sales
            END sls_sales, -- Recalculte sales if original value is missing or incorrect
            ...
            CASE WHEN sls_price IS NULL OR sls_price <= 0
                ...
                ELSE sls_price
            END sls_price -- Recalculte sales if original value is missing or incorrect
        ...
        SELECT
            ...
            cat,
            subcat,
            REPLACE(maintenance, CHAR(13), '') maintenance
